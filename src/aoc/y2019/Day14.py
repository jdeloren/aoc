...
def stacker(stockpile, unused, stack, formulae, source, target):
    ...
    while len(stack) > 0:
        ...
        if material[2] == source:
            ...
            # todo - div remainder isn't leveraged
            div = 1 if item[0] < material[0] else item[0] // material[0]
            extra = item[0] % material[0]
            print(f"LEFTOVERS: {extra} for generating {item[1]} with {material[2]}")
